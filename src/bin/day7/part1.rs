...
fn solve(input: &str) -> u64 {
    ...
    let mut layer = HashSet::from([(map.start.0, map.start.1)]);

    while !layer.is_empty() {
        println!("Layer: {layer:?}");
        let mut next_layer = HashSet::new();

        for (row, col) in layer.iter().copied() {
            ...
            match bottom {
                ...
                Cell::Empty | Cell::Tachyon => {
                    ...
                    next_layer.insert((row + 1, col));
                }
                Cell::Splitter => {
                    nsplits += 1;
                    map.set(row + 1, col - 1, Cell::Tachyon);
                    map.set(row + 1, col + 1, Cell::Tachyon);
                    next_layer.insert((row + 1, col - 1));
                    next_layer.insert((row + 1, col + 1));
                }
            }
        }
...
#[test]
fn test() {
    assert_eq!(solve(include_str!("input0")), 21)
}
